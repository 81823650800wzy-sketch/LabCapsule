...
LabCapsule 是基于 ESP32-S3 的便携式 AI 辅助实验组件。当前版本为 **V0.3.1 Alpha / Motion Experiment Prototype**，已经打通“自然语言问题 → 实验协议 → 六轴采集 → CSV/分析”的设备侧和 Android 控制链路。
...
2. 安装 `release/LabCapsule-0.3.1.apk`。首次进入“设备”，选择：
...
完整操作见 [V0.3.1 中文使用指南](docs/V0.3.1_USER_GUIDE_ZH.md)，系统边界与扩展方式见 [V0.3.0 架构说明](docs/V0.3.0_ARCHITECTURE.md)。
...
2026-08-26 已在 COM8 的 ESP32-S3 rev 0.2 / 16 MiB Flash / 8 MiB Octal PSRAM 上完成 V0.3.1 构建、烧录和串口启动回归。ST7789 驱动、PSRAM、Wi-Fi AP+STA、BLE 和媒体协议均正常初始化。I²C 地址 `0x68` 已有设备应答，但实测 `WHO_AM_I=0x70` 与标准 MPU6050 标识不一致，正式采集前仍应确认模块型号和接线。

## 安全边界

- APK 中的 API Key、Wi-Fi 和 MQTT 密码使用 Android Keystore 加密保存。
- 设备状态接口不回传密码；MQTT 支持 `mqtts://` 并使用 ESP-IDF CA 证书包。
- 恢复热点与局域网 HTTP API 面向受信任本地网络，V0.3.1 尚未提供逐设备 HTTP 登录或云端中继服务。
