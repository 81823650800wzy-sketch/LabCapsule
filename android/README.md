# LabCapsule Android Remote 0.3.2

原生 Java Android 8.0+ 控制器，不依赖 Gradle 或第三方运行库。界面默认中文，采用五分区透明液态滚轮导航：设备、屏幕、实验、AI、设置。

V0.3.2 使用原创工业街机视觉系统：高对比警示色、信息标签、斜线底纹和透明控制甲板。它与设备端 ST7789 使用同一套三色预设和信息层级，不包含第三方游戏的 logo、角色或素材。
...
输出：`dist/LabCapsule-0.3.2.apk`。脚本会完成资源编译、Java 编译、DEX、APK 对齐、签名及签名验证。开发签名文件位于本机并被 `.gitignore` 排除。
